...
solv_struct getsolve(double size)
{
    ...
    std::vector<double> Sj(N / 2.0);

    double E = 0;

    // коэффициент Aq
    for (double q = 1; q < N / 2.0 - 1; q++) {
        E = 0.0;
        for (double i = 1; i < N; i++) {
            E += Ex.at(i) * (cos((2.0 * 3.14159 * q * i) / N));
        }
        Aq[q] = E * (2.0 / N);
    }
    ...
    for (double q = 1; q < N / 2.0 - 1; q++) {
        E = 0.0;
        for (double i = 1; i < N; i++) {
            E = E + Ex.at(i) * (sin((2.0 * 3.14159 * q * i) / N));
        }
        Bq[q] = E * (2.0 / N);
    }

    // Вычисление спектральной плотности мощности
    for (double i = 1; i < N / 2.0 - 1; i++) {
        Pq[i] = (Aq.at(i) * Aq.at(i) + Bq.at(i) * Bq.at(i));
        Sj[i] = Pq[i];
    }

    // Применение цифрового фильтра Хэмминга
    for (int i = 2; i < N / 2.0 - 2; i++) {
        Sj[i-2] = 0.23 * Pq.at(i - 1) + 0.54 * Pq.at(i) + 0.23 * Pq.at(i + 1);
    }
    ...
    if (saveMsgBox.exec() == QMessageBox::Yes) {
        ...
        try {
            ...
            for (int i = 0; i < Sj.size(); ++i) {
                updateQuery.bindValue(":value", Sj[i]);
                updateQuery.bindValue(":id", minId + i);

                qDebug() << "Setting value" << Sj[i] << "for id" << (minId + i);
